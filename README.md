...
The first runnable slice covers login, scenes/subscenes/rounds, durable jobs, material partition isolation, document revisions, alignment concurrency semantics, five asset states, and cumulative partial releases. Binary ingest/parsing, real knowledge generation, asset renderers, scene exploration and sandboxed Skill execution remain separately versioned capabilities; see [the implementation status](docs/implementation-status.md).
...
The approved source artifacts are preserved under `docs/product/originals/`. Their checksums are recorded in `SHA256SUMS`.
